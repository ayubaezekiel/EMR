...
export const useAppointmentPaginatedQuery = (
  page: number,
  pageSize: number
) => {
  return useQuery({
    queryKey: ["paginatedData", page, pageSize],
    queryFn: async () => {
      ...
      const { data, error, count } = await supabase
        .from("appointments")
        .select(
          "*,patients(id,first_name,middle_name,last_name),clinics(name),appointment_types(*)"
        )
