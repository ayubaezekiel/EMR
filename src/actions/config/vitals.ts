...
export const createVitalsAction = async (values: DB["vitals"]["Insert"]) => {
	const { error, data } = await supabase.from("vitals").insert(values);
	if (error && !data) {
...
export const deleteVitalsAction = async ({ id }: { id: string }) => {
	if (id) {
		const { error, data } = await supabase.from("vitals").delete().eq("id", id);
		if (error && !data) {
			toast.error(error.message);
		}
...
export const createPatientVitalsAction = async (
	...
	const { error, data } = await supabase.from("patient_vitals").insert(values);
	if (error && !data) {
		toast.error(error.message);
	} else {
