...
export const updateVitalsAction = async (values: DB['vitals']['Update']) => {
    if (values.id) {
        const { error } = await supabase.from("vitals").update(values).eq('id', values.id);
        if (error) {
            toast.error(error.message);
        }
...
export const createPatientVitalsAction = async (values: DB['patient_vitals']['Insert']) => {
...
export const updatePatientVitalsAction = async (values: DB['patient_vitals']['Update']) => {
    if (values.id) {
        const { error } = await supabase.from("patient_vitals").update(values).eq('id', values.id);
        if (error) {
            toast.error(error.message);
        }
...
export const deletePatientVitalsAction = async ({ id }: { id: string }) => {
    if (id) {
        const { error } = await supabase.from("patient_vitals").delete().eq('id', id);
        if (error) {
            toast.error(error.message);
        }
