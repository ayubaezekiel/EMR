...
export const createImagingAction = async (
    values: DB['imaging']["Insert"]
) => {
...
export const updateImagingAction = async (
    values: DB['imaging']["Update"]
) => {
    if (values.id) {
...
export const deleteImagingCategoryAction = async ({ id }: { id: string }) => {
    if (id) {
        const { error } = await supabase.from("imaging_category").delete().eq('id', id);
        if (error) {
            toast.error(error.message);
        }
...
export const deleteImagingTemplateAction = async ({ id }: { id: string }) => {
    if (id) {
        const { error } = await supabase.from("imaging_templates").delete().eq('id', id);
        if (error) {
            toast.error(error.message);
        }
