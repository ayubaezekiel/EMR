    ...
    <div>
      ...
      <Tabs.Root defaultValue="waiting" my={"4"}>
        ...
        <Tabs.Content value="waiting" mt={"2"}>
          <PharmRequestCard isCompleted={false} isWaiting={true} />
        </Tabs.Content>

        <Tabs.Content value="completed" mt={"2"}>
          <PharmRequestCard isCompleted={true} isWaiting={false} />
        </Tabs.Content>
      </Tabs.Root>
    </div>
  );
...
interface PharmRequestStatus {
  isWaiting?: boolean;
  isCompleted?: boolean;
}
function PharmRequestCard({ isCompleted, isWaiting }: PharmRequestStatus) {
  ...
  const pharm_request_waiting = request_data?.request_data?.filter(
    (a) => a.is_waiting === isWaiting && a.is_lab === true
  );

  const pharm_request_completed = request_data?.request_data?.filter(
